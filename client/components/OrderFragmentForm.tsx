...
interface OrderFragmentFormProps {
  ...
  totalValue: number;
  onSave: (fragments: OrderFragment[]) => void;
  onCancel: () => void;
}
...
export default function OrderFragmentForm({ 
  ...
  const [fragments, setFragments] = useState<Partial<OrderFragment>[]>([
    {
      fragmentNumber: 1,
  ...
  const handleSave = () => {
    ...
    const completeFragments: OrderFragment[] = fragments.map((fragment, index) => ({
