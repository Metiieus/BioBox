...
import { ProductionTask, productionStages, mockOperators } from "@/types/production";
...
export default function NewTaskForm({ onSave, onCancel }: NewTaskFormProps) {
  ...
  const availableOperators = mockOperators.filter(op => 
    !selectedStage || selectedStage.requiredSkills.some(skill => op.skills.includes(skill))
  );
