...
import { mockOperators } from "@/types/production";
...
export default function ProductionReport({ onClose }: ProductionReportProps) {
  ...
  const getReportData = () => {
    ...
    const operatorEfficiency = mockOperators.reduce((sum, op) => sum + op.efficiency, 0) / mockOperators.length;
...
OPERADORES:
${mockOperators.map(op => `- ${op.name}: ${op.efficiency}% eficiência (${op.status})`).join('\n')}
            ...
            <CardContent>
              <div className="space-y-3">
                {mockOperators.map(operator => (
                  <div key={operator.id} className="flex items-center justify-between p-3 border border-border rounded-lg">
                    <div className="flex items-center space-x-3">
                      <div className="w-8 h-8 bg-biobox-green/10 rounded-full flex items-center justify-center">
                        <span className="text-xs font-medium text-biobox-green">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {operator.experience} anos • {operator.skills.length} habilidades
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      >
                        {operator.status === 'available' ? 'Disponível' : 'Ocupado'}
                      </Badge>
                    </div>
                  </div>
                ))}
