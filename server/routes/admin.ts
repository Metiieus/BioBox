...
export const createUserAdmin: RequestHandler = async (req, res) => {
  try {
    ...
    if (createErr || !created?.user) {
      return res
        .status(500)
        .json({ error: "create_user_failed", details: createErr?.message });
    }
    ...
    if (upsertErr) {
      return res
        .status(500)
        .json({ error: "profile_upsert_failed", details: upsertErr.message });
    }
    ...
    return res
      .status(500)
      .json({ error: "internal_error", details: e?.message });
  }
};
